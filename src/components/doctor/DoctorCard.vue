...
<template>
	...
	>
		<div class="relative z-10 py-5 pl-4 pr-39 space-y-0.5 text-gray">
			...
			<div v-if="times.length" class="flex flex-wrap gap-1.5 pt-2">
				<span
					v-for="time in times"
					:key="time"
					class="flex items-center justify-center min-h-8 py-1 px-2 rounded-full border border-brand text-13 text-brand"
				>
					{{ time }}
				</span>
			</div>
