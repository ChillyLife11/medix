...
>   плюс перечисление статусов: главная просит `filter[status][]=0…4`, история —
...
- **ФИО врача перепутаны местами** в ответе бэка: фамилия лежит в
  ...
  именно `profile`.
- **Карточке врача нужен `isolate`.** Текстовый блок внутри лежит на `z-10`, а
  сама карточка — `relative` без `z-index`, то есть своего контекста наложения не
  создаёт: блок всплывал в корневой и перекрывал липкую кнопку внизу экрана,
  перехватывая тапы (кнопка «Выбрать врача» не срабатывала, если под ней
  проходила карточка). У кнопки на этом экране для верности тоже `z-10`.
  Проверено: без правки тап уходит в карточку, с ней — в кнопку.
