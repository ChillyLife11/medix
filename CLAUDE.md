...
- **Отладочные тосты по API — временные.** `DEBUG_HTTP` в `config.js`,
  ...
  **Флаг включён, в том числе в прод-сборке** — отлаживаем именно её. Перед
  релизом выключить (`DEBUG_HTTP = false`) или удалить `components/debug`
  вместе с `useHttpLog`. `access_token` в выводе маскируется: остаются первые 6 символов
  и длина.
